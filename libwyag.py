...
class GitRepository (object):
    ...
    def __init_(self, path, force=False):
        self.worktree = path
    ...
    def repo_create(path):
        ...
        #Make sure the path doesn't exist or is an empty dir
        
        if os.path.exists(repo.worktree):
            if not os.path.isdir(repo.worktree):
                raise Exception ("% is not a directory!" % path)
            if os.path.exists(repo.gitdir) and os.lisdir(repo.gitdir):
    ...
    def repo_find(path=".", required=True):
        ...
        #If there has been no returned, recurse in parent directory
        if os.path.realpath(os.path.join(path, ".."))
    ...
    class GitObject (object):
        ...
        def serialize(self, repo):
            ...
            whatever it takes to convert into a meaningful representation.
    ...
    argsp = argsubparsers.add_parser("cat_file,
